...
class HeroPlane(BasePlane):#定义主机
    ...
    def shoot(self):  # 主机发射三个等级的子弹
        ...
        elif 5000 < score <= 10000:# 当积分达到5000分时，子弹升级
...
class BigEnemyPlane(EnemyPlane):
    ...
    def move(self):
        if self.y >= WINDOW_HEIGHT * 2:
            ...
            self.x = random.randint(0, WINDOW_WIDTH - self.self_x)
            self.enemy_bullet()
    # 当积分达到1000时，敌机加速；积分达到20000时，敌机再加速
        if score < 10000:
            self.y += 1
        if 10000 <= score < 20000:
            self.y += 2
...
class BeiJing:#背景类
    ...
    def start(self):
        ...
        while True:
            ...
            text_obj = font_obj.render("ENTER开始  ESC退出", 1, (0, 255, 0))
            text_obj2 = font_obj2.render("飞 机 大 战    ", 1, (255,0, 0))
    ...
    def die(self):
        ...
        while True:
            ...
            font_obj = pygame.font.Font("res/SIMHEI.TTF", 42)
            font_obj2 = pygame.font.Font("res/SIMHEI.TTF", 60)
            text_obj = font_obj.render("ENTER重新开始  ESC退出", 1, (255, 255, 255))
            text_obj2 = font_obj2.render("Game Over", 1, (255, 0, 0))
            text_rect = text_obj.get_rect(centerx=260, centery=380)
